#!groovy
    ...
    node(javaAgent) {
        ...
        try {
			...
			sh "sudo docker run --restart=always --network=host --name=${projectName} -v /opt/mylibrary:/opt/app/mylibrary-h2-db -e CONFIG_SERVER_URI=http://${params.configServerIp}:8888 -td ${projectName}"
			}
        } catch (def e) {
